...
export const CHUNK_NARRATION_PROMPT_VERSION = 'narration-chunk-2';

/**
 * The chunk-narration + badge prompt (spec 06 slice 5; note added spec-less under R-067/R-068). One
 * file's aliased chunks in, per-chunk `{ line, badge, note }` out. New prompt track (spec 03's
 * `narration-4` is untouched); it bakes in the point-don't-assert rule (#58) — lines and notes POINT
 * at what to check, they never assert a semantic outcome that only holds on one branch. The `note` is
 * the adaptive-depth valve: a deeper "what to verify" paragraph, emitted ONLY for genuinely complex
 * chunks. Bump CHUNK_NARRATION_PROMPT_VERSION on any edit; the overlay records it and resume compares it.
 */
...
the book once, top to bottom. For each chunk you may write up to three things: a short badge, a
short orientation line, and — rarely — a deeper review note. All are optional per chunk — be sparse.
...
3. A note (RARE — most chunks must not get one): a deeper "what to verify" paragraph, at most 3
   short sentences, only for a chunk whose logic is genuinely complex, subtle, or easy to get wrong
   — a non-obvious invariant, a hand-maintained coupling two places must keep in sync, a tricky edge
   case, an ordering the code depends on. Use it to give the reviewer the extra context a hard chunk
   needs and to name exactly what they should confirm. It still POINTS — it never says the code is
   right. If a chunk is ordinary, OMIT the note. A note on a simple chunk is a defect. Expect at most
   one or two notes across a whole file, often none.

Rules for every line and badge and note:
...
{"c1": {"line": "<line>", "badge": "<badge>"}, "c2": {"badge": "<badge>", "note": "<note>"}}

Include only the aliases you chose to label; omit every other chunk. Each entry may have "line",
"badge", "note", or any combination. Use the aliases exactly as shown below.
