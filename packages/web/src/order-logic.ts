...
/**
 * Whether the AI order overlay should be applied, offered, or left alone. The server already
 * discards stale (fingerprint-mismatched) overlays, so a non-null overlay here is always fresh.
 * `appliedAt`/`dismissedAt` are sticky once set — a mark made after auto-apply must never flip
 * a displayed order back to tier 0.
 */
