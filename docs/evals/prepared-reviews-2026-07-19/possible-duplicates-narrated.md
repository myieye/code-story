...
111 chunks · 15 sections · head 2dc41fb3

> AI: This change adds duplicate detection to the new-entry flow, with matching logic, a check panel, a summary pill, and dialog wiring. Follow the thread from duplicate-check.ts, since the panel, pill, and dialog all build on how it finds and classifies matches.

## frontend/viewer/src/lib/components/dictionary/DictionaryEntry.svelte

> AI: Adds an `inline` prop to render senses on one flowing line instead of a break per sense. Check the prop plumbing and the template branch.
...
> AI: When `inline`, a space replaces the `<br />` between senses.
...
> AI: New helper module for entry duplicate detection: query building, normalization, morph-token stripping, and match classification. Check the rules mirror the backend search.
...
> AI: Scores each morph type by prefix/postfix hits, then strips the best one from the value.
...
> AI: Dedupes queries per field kind, not across, and skips values below MIN_QUERY_LENGTH.
...
> AI: Precomputes normalized query forms, then classifies and sorts each candidate.
...
> AI: Checks headword/lexeme exact match first, then similar-word by length delta, then gloss meaning match.
...
> AI: New test file for the duplicate-check helpers: normalize/diacritics, morph-token stripping, query building, result merging, and duplicate classification.
...
> AI: New duplicate-check widget for the entry editor. Look at how it searches, classifies matches, and lets users jump to or add a sense to an existing entry.
...
> AI: Debounced search over-fetches, then aborts stale results superseded by newer keystrokes.
...
> AI: Classification runs outside the resource and feeds the exported summary via an effect.
...
> AI: Auto-expands on an exact word match unless the user has toggled; clears state when matches empty.
...
> AI: addSenseToEntry clones the sense with a fresh id before saving to the target entry.
...
> AI: New pill component that summarizes duplicate matches with a jump button and a dismiss button.
...
> AI: This file wires the new-entry dialog to a duplicate-check widget and a summary pill. Watch the createEntry guard against double-create and the busy-state gating.

### script

other · +22 -1

> AI: Adds duplicate state, a jump helper, and an IntersectionObserver tracking whether the widget is on screen.
...
> AI: Body wrapped in try/finally; loading and duplicateActionBusy now block re-entry before the first await.
...
> AI: Expands the check, waits a tick, then scrolls the widget into view.
...
> AI: Mounts DuplicateCheck, shows a sticky pill when the widget is off screen and not dismissed.
...
> AI: New Playwright suite for the new-entry duplicate check: matching a word/gloss, the new-word indicator, expanding rows, and the jump pill. Check selectors and the demo-data assumptions.
...
> AI: Tests navigating to a matched entry and the new-word indicator; note the 'baba' substring assumption.
...
> AI: Tests adding a sense to an existing entry (polls the API) and the collapsed partial-match strip.
...
> AI: Tests the capped 'N+' count with Show more, the gloss-match badge, and the out-of-view jump pill setup.
...
> AI: Clicking the pill scrolls the strip into view and hides the pill.
...
> AI: Adds English strings for the duplicate-check feature in New Entry/New Word dialogs. Check the Classic vs Lite wording pairs and the translator context comments.
